# Deploy — GrivachevskyDetailing v1.1.2

1. Upload the repository contents to GitHub repository `GrivachevskyDetailing`.
2. Connect that repository to Cloudflare Worker `grivachevsky-detailing`.
3. Add `BOT_TOKEN` and `SESSION_SECRET` as secrets.
4. Optionally add a valid `TELEGRAM_WEBHOOK_SECRET`.
5. Bind D1 database `grivachevsky-detailing` as `DB`.
6. Deploy with `npx wrangler deploy`.
7. Open the deployed Mini App once. This immediately self-heals the Telegram webhook.
8. Check `/api/telegram/health` and then send `/start` to `@GrivachevskyDetailing_bot`.
