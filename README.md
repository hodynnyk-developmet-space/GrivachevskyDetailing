...
- When direct URL access is OFF, opening the Worker URL outside Telegram shows a polished Grivachevsky screen that sends the visitor to `@GrivachevskyDetailing_bot`.
...
Production-oriented Telegram Mini App + Cloudflare Worker for **@GrivachevskyDetailing_bot**.
...
Secrets:
- `BOT_TOKEN` — token from BotFather for `@GrivachevskyDetailing_bot`
...
1. Add `@GrivachevskyDetailing_bot` to the target group/chat.
