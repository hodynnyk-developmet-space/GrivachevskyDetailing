...
export default {
 async fetch(request:Request,env:Env,ctx:ExecutionContext):Promise<Response>{
  ...
  try{
   ...
   if(url.pathname==='/__version')return json({app:'GrivachevskyDetailing',version:VERSION,database:'grivachevsky-detailing',worker:true});
   ...
   if(url.pathname==='/telegram/fix'&&request.method==='GET'){
    ...
    return html(`<h1>Grivachevsky Detailing — Bot Fix</h1><p class="${cls}">${result.ok?'✅ Webhook repaired successfully':'❌ Webhook is still not correct'}</p><pre>${escapeHtml(JSON.stringify(result,null,2))}</pre><p><a href="${escapeHtml(result.testBotUrl)}">Open @GrivachevskyDetailing_bot and test /start</a></p><p>After it works, rotate or remove <code>TELEGRAM_SETUP_KEY</code>.</p>`,result.ok?200:500);
   }
   ...
   if(url.pathname==='/api/system/status'){
    ctx.waitUntil(selfHealWebhook(env,url.origin));
    const maintenance=(await getSetting(env,'maintenance.enabled','0'))==='1';
    return json({maintenance,app:env.APP_NAME,version:VERSION,dbConfigured:!!env.DB,botConfigured:!!env.BOT_TOKEN,webhookEnabled:true,webhookEndpoint:'/api/telegram/webhook',appUrlConfigured:!!env.APP_URL,databaseName:'grivachevsky-detailing'});
   }
   ...
   if(url.pathname==='/api/auth/telegram'&&request.method==='POST'){
    ...
    if(!hasTelegramInit&&(await getSetting(env,'web_direct_access_enabled','0'))!=='1'){
     return json({error:'DIRECT_WEB_DISABLED',botUsername:String(env.BOT_USERNAME||'GrivachevskyDetailing_bot').replace(/^@/,'')},403);
    }
   ...
   if(url.pathname==='/api/referrals/create'&&request.method==='POST'){
    ...
    if(u.demo)return json({ok:true,code:'DEMO',url:`https://t.me/${String(env.BOT_USERNAME||'GrivachevskyDetailing_bot').replace(/^@/,'')}?start=ref_DEMO`});
    ...
    const bot=String(env.BOT_USERNAME||'GrivachevskyDetailing_bot').replace(/^@/,'');
