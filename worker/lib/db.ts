...
CREATE TABLE IF NOT EXISTS specialists(id INTEGER PRIMARY KEY AUTOINCREMENT,name TEXT NOT NULL,role_title TEXT,portfolio_url TEXT,contact_url TEXT,contact_label TEXT,photo_url TEXT,enabled INTEGER NOT NULL DEFAULT 1,sort_order INTEGER NOT NULL DEFAULT 0,created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP);
CREATE TABLE IF NOT EXISTS client_feedback(id INTEGER PRIMARY KEY AUTOINCREMENT,user_id INTEGER NOT NULL,kind TEXT NOT NULL CHECK(kind IN ('REVIEW','SUGGESTION')),text TEXT NOT NULL,photo_file_id TEXT,language TEXT,status TEXT NOT NULL DEFAULT 'NEW',created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP);
...
CREATE TABLE IF NOT EXISTS bot_ui_state(chat_id INTEGER PRIMARY KEY,panel_message_id INTEGER,updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP);
...
CREATE TABLE IF NOT EXISTS manager_permissions(permission_key TEXT PRIMARY KEY,enabled INTEGER NOT NULL DEFAULT 0,updated_by INTEGER,updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP);
CREATE TABLE IF NOT EXISTS personal_offers(id INTEGER PRIMARY KEY AUTOINCREMENT,user_id INTEGER NOT NULL,created_by INTEGER NOT NULL,created_by_role TEXT NOT NULL,status TEXT NOT NULL DEFAULT 'DRAFT',vehicle_slug TEXT NOT NULL DEFAULT 'sedan',condition_slug TEXT NOT NULL DEFAULT 'normal',final_price REAL,currency TEXT NOT NULL DEFAULT 'PLN',note TEXT,requires_approval INTEGER NOT NULL DEFAULT 0,approved_by INTEGER,approved_at TEXT,sent_at TEXT,accepted_at TEXT,request_id INTEGER,created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP);
CREATE TABLE IF NOT EXISTS personal_offer_services(id INTEGER PRIMARY KEY AUTOINCREMENT,offer_id INTEGER NOT NULL,service_id INTEGER NOT NULL,is_primary INTEGER NOT NULL DEFAULT 0,UNIQUE(offer_id,service_id));
CREATE TABLE IF NOT EXISTS service_request_extras(id INTEGER PRIMARY KEY AUTOINCREMENT,request_id INTEGER NOT NULL,service_id INTEGER NOT NULL,service_slug TEXT NOT NULL,title_snapshot TEXT,price_snapshot REAL NOT NULL,currency TEXT NOT NULL,added_by INTEGER,created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP);
CREATE INDEX IF NOT EXISTS idx_personal_offers_user ON personal_offers(user_id,status,created_at);
CREATE INDEX IF NOT EXISTS idx_request_extras_request ON service_request_extras(request_id);
...
`);
 ...
 await safeAlter(env,"ALTER TABLE service_option_translations ADD COLUMN description TEXT NOT NULL DEFAULT ''");
...
async function seed(env:Env){
 ...
 const botMenuCopy={
  uk:{'bot.client_menu_text':`Ласкаво просимо до Grivachevsky Detailing 🦎

Усі послуги, розрахунок і заявки зібрані в Mini App. Якщо потрібна допомога — ми поруч.`,'bot.help_text':`💚 <b>Потрібна допомога?</b>

Напишіть власнику Grivachevsky Detailing або змініть мову бота — оберіть потрібну дію нижче.`},
  pl:{'bot.client_menu_text':`Witamy w Grivachevsky Detailing 🦎
...
Napisz do właściciela Grivachevsky Detailing lub zmień język bota — wybierz opcję poniżej.`},
  en:{'bot.client_menu_text':`Welcome to Grivachevsky Detailing 🦎
...
Message the owner of Grivachevsky Detailing or change the bot language — choose an option below.`},
  de:{'bot.client_menu_text':`Willkommen bei Grivachevsky Detailing 🦎
...
Schreibe dem Inhaber von Grivachevsky Detailing oder ändere die Bot-Sprache — wähle unten eine Option.`},
  fr:{'bot.client_menu_text':`Bienvenue chez Grivachevsky Detailing 🦎
...
Contactez le propriétaire de Grivachevsky Detailing ou changez la langue du bot — choisissez une option ci-dessous.`}
 } as const;
