...
const button=(env:Env)=>{const url=appUrl(env);return url?{inline_keyboard:[[{text:'🦎 Grivachevsky Detailing',web_app:{url}}]]}:undefined};
...
export async function runReactivationCampaigns(env:Env,force=false){
 ...
 const text=await getSetting(env,'reactivation.message','Ми давно не бачились 🦎 Якщо авто знову потребує уваги — Grivachevsky Detailing поруч.');
