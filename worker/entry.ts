...
export default {
 async fetch(request:Request,env:SheetsEnv,ctx:ExecutionContext):Promise<Response>{
  ...
  if(url.pathname==='/__version')return json({app:'GrivachevskyDetailing',version:VERSION,database:'grivachevsky-detailing',worker:true,googleSheetsAppsScript:true});
