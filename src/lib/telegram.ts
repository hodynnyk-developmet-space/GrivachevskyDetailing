...
export const openBot=(start?:string)=>{const username='GrivachevskyDetailing_bot';const url=`https://t.me/${username}${start?`?start=${encodeURIComponent(start)}`:''}`;const w=tg();if(w?.openTelegramLink)w.openTelegramLink(url);else window.open(url,'_blank','noopener,noreferrer')};
